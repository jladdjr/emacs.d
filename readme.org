...
#+end_src
** Do not highlight trailing whitespace when calling vc-annotate

#+begin_src emacs-lisp
 (advice-add 'vc-annotate
             :after
             (lambda (&rest args)
...
Use symbolic links to map files from other directories into this location.
...
#+begin_src emacs-lisp
  ...
  (use-package org-download
    :straight (:type git
                     :host nil
                     :repo "https://github.com/jladdjr/org-download.git"
                     :branch "main")
    :config
...
Install [[https://github.com/dominikh/go-mode.el][go-mode]].

#+begin_src emacs-lisp
  (unless skip-dev-config
    (message "load go-mode")
    (use-package go-mode))
#+end_src
...
Install [[https://github.com/galdor/rfc-mode][rfc-mode]] for reading [[https://www.rfc-editor.org/][RFCs]].

Call ~rfc-mode-browse~ to view a list of RFCs.

RFCs will automotically downloaded to ~\~/rfc~).
The destination folder can be changed by setting ~rfc-mode-directory~).

#+begin_src emacs-lisp
  (message "load rfc-mode")
  (use-package rfc-mode)

  (defun jl/rfc-mode-disable-whitespace ()
    "Disable trailing whitespace and hl-line in Rfc-Mode buffers."
    (setq show-trailing-whitespace nil)
    (hl-line-mode 0))

  (add-hook 'rfc-mode-hook #'jl/rfc-mode-disable-whitespace)
#+end_src
...
#+begin_src emacs-lisp
  ...
  (use-package elfeed
    ...
    (setq elfeed-feeds '("https://github.blog/engineering.atom"
                         ...
                         "https://www.youtube.com/feeds/videos.xml?channel_id=UCoSPSd6Or4F_vpjo4SmyoEA"))
...
#+begin_src emacs-lisp
  (use-package wttrin
    :straight (:type git
                     :host nil
                     :repo "git@github.com:jladdjr/emacs-wttrin.git"
    ...
    (use-package vterm
        ...
        (add-hook 'vterm-mode-hook
                (lambda ()
                    ...
                    (setq show-trailing-whitespace nil)
                    ; Do not highlight nobreak spaces (U+OOA0)
                    ; (such as those shown by `tree`)
                    ; https://emacsdocs.org/docs/emacs/Text-Display
                    ; https://unicodeplus.com/U+00A0
...
** Install gemtext-mode

Install [[https://github.com/emacsmirror/gemtext-mode][gemtext-mode]].
...
Install [[https://github.com/abrochard/mermaid-mode][mermaid-mode]].

Mode is automatically activated with [[https://github.com/abrochard/mermaid-mode/blob/master/mermaid-mode.el#L271][*.mmd]] files.

#+begin_src emacs-lisp
  (unless skip-dev-config
    (message "load mermaid-mode")
    (use-package mermaid-mode))
#+end_src

** Install d2-mode

Install [[https://github.com/andorsk/d2-mode][d2-mode]].

#+begin_src emacs-lisp
  (unless skip-dev-config
    (message "load d2-mode")
    (use-package d2-mode))
#+end_src
...
#+begin_src emacs-lisp
  (defun convert-epoch-at-point ()
    (interactive)
    (let* ((bounds (bounds-of-thing-at-point 'word))
      (epoch (string-to-number
              (buffer-substring-no-properties
               (car bounds) (cdr bounds))))
      (date (format-time-string
             "%Y-%m-%d %H:%M:%S"
             (seconds-to-time (/ epoch 1000)))))
